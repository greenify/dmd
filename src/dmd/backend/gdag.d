...
alias AEcse = Aetype.AEcse;
alias AEarraybounds = Aetype.AEarraybounds;
__gshared Aetype aetype;
...
//#else
        ...
        for (uint i = 0; i < dfotop; i++)
        {
                ...
                /* expressions.                                         */
                version(SCPP)
                    auto scppCondition = b.BC == BCcatch;
                else
                    enum scppCondition = false;
                version(MARS)
                    auto marsCondition = b.BC == BCjcatch;
                else
                    enum marsCondition = false;
                if ((i != 0 &&
                     (list_block(b.Bpred) != dfo[i - 1] ||
                      b.Bpred.next !is null))
                    || b.BC == BCasm
                    || b.BC == BC_finally
                    || b.BC == BC_lpad
                    || scppCondition
...
{       vec_t aer;
        ...
        {   // Try to find an equivalent AE, and point to it instead
            ...
            if (aetype == AEcse)
            {
                for (i = cast(uint) vec_index(0, ae); i < go.exptop; i = cast(uint) vec_index(++i, ae))
                {   elem *e = go.expnod[i];

                    // Attempt to replace n with e
                    if (e is null)              // if elem no longer exists
        ...
        if (OTdef(op))
        {
                ...
                if (OTassign(n.Eoper) && n.EV.E1.Eoper == OPvar)        // if unambiguous definition
                {       Symbol *s;
