...
private extern (C++) final class ExpressionSemanticVisitor : Visitor
{
    ...
    override void visit(IsExp e)
    {
        ...
        else if (e.tspec && !e.id && !(e.parameters && e.parameters.dim))
        {
            ...
            if (e.tok == TOKcolon)
            {
                ...
                auto oldatlock = e.targ.aliasthislock;
                e.targ.aliasthislock |= RECtracing;
    ...
    override void visit(NotExp e)
    {
        ...
        if (e.e1.op == TOKtype)
            e.e1 = resolveAliasThis(sc, e.e1, 0); // TODO
    ...
    override void visit(AssignExp exp)
    {
         ...
         */
        {
            ...
            // TODO
            if (e2x.op == TOKtype)
                e2x = resolveAliasThis(sc, e2x, 0); //https://issues.dlang.org/show_bug.cgi?id=17684
            if (e2x.op == TOKerror)
            {
                result = e2x;
                return;
                ...
                if (e1x.op == TOKindex && (cast(IndexExp)e1x).e1.type.toBasetype().ty == Taarray)
                {
                    ...
                    if (e)
                    {
                        ...
                        if (ey)
                        {
                            ...
                            Type t = null;
                            // TODO
                            if (!typeMerge(sc, TOKquestion, &t, &ex, &ey, false))
                            {
    ...
    override void visit(LogicalExp exp)
    {
        ...
        // for static alias this: https://issues.dlang.org/show_bug.cgi?id=17684
        if (e1x.op == TOKtype)
            e1x = resolveAliasThis(sc, e1x, 0); // TODO
